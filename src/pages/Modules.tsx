...
import { createClassroomCatalogSupabaseFirst } from '../services/classroomCatalogService';
...
export const Modules: React.FC = () => {
  ...
  const fetchCurriculum = async (bypassCache = false) => {
    ...
    try {
      ...
      const { supabaseModules, aiSuggestions } = await createClassroomCatalogSupabaseFirst({
        grade,
        selectedBacTrackId,
        generateAiSuggestions: aiAvailable
          ? async () => {
      ...
      if (supabaseModules.length === 0 && aiSuggestions.length > 0) {
            ...
            <div className="group relative inline-block">
              ...
              <div className="absolute right-0 top-full mt-2 w-72 p-4 bg-paper border border-ink/5 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                <p className="text-sm text-muted leading-relaxed">
                  These classrooms load from Supabase curriculum data first. AI can optionally suggest extra modules.
                </p>
              </div>
            </div>
          </div>
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted/40 max-w-md mx-auto leading-relaxed">
                    Click 'Create' to load your classroom from Supabase curriculum data for {grade}{bacTrackName ? ` - ${bacTrackName}` : ''}{bacIntOptionName ? ` (${bacIntOptionName})` : ''} in {country}.
                  </p>
                </div>
                {!aiAvailable && (
