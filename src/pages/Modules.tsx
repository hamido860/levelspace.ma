...
import { buildModulesFromLessons, buildSupabaseLessonFilters } from '../services/classroomService';
...
export const Modules: React.FC = () => {
  ...
  const aiUnavailableMsg = "Guided suggestions need an API key, but classroom content is still available.";
  ...
  const loadClassroomsFromSupabase = async () => {
    setIsLoading(true);
    try {
      let query = supabase
        .from('lessons')
        .select('subject, mod')
        .limit(500);
      const filters = buildSupabaseLessonFilters(grade, country);
      for (const filter of filters) {
        query = query.eq(filter.column, filter.value);
      }
      const { data, error } = await query;
      if (error) throw error;
      const baseModules = buildModulesFromLessons(data || []);
      if (baseModules.length > 0) {
        await db.modules.bulkPut(baseModules);
      }
    } catch (error) {
      console.error('Failed to load classrooms from Supabase:', error);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1 flex-1">
            <div className="flex items-start md:items-center gap-2 text-accent">
              ...
              <span className="text-[10px] font-bold uppercase tracking-[0.3em] leading-relaxed">Curriculum-aligned for {grade}{bacTrackName ? ` - ${bacTrackName}` : ''}{bacIntOptionName ? ` (${bacIntOptionName})` : ''} in {country}</span>
            </div>
            ...
            <div className="group relative inline-block">
              ...
              <div className="absolute right-0 top-full mt-2 w-72 p-4 bg-paper border border-ink/5 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                <p className="text-sm text-muted leading-relaxed">
                  Load classrooms from Supabase first. Use guided suggestions only when you need extra ideas.
                </p>
              </div>
            </div>
          </div>
            ...
            {isLoading ? (
              ...
              >
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-serif">Your classroom is ready.</h3>
                  <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted/40 max-w-md mx-auto leading-relaxed">
                    Load your classroom subjects from Supabase for {grade}{bacTrackName ? ` - ${bacTrackName}` : ''}{bacIntOptionName ? ` (${bacIntOptionName})` : ''} in {country}.
                  </p>
                </div>
                {!aiAvailable && (
                ...
                <button
                  onClick={loadClassroomsFromSupabase}
                ...
                <button
                  ...
                  title={!aiAvailable ? aiUnavailableMsg : undefined}
                  className="px-8 py-3 bg-surface-low text-ink rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-accent/10 transition-all flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                >
