        ...
        {[...mainNavItems, ...toolNavItems].map((item) => {
          ...
          return (
            <button
              ...
              onClick={() => navigate(item.path)}
              aria-label={item.label}
              className={`flex flex-col items-center justify-center gap-1 h-full px-3 min-w-[72px] shrink-0 transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1 focus-visible:rounded-lg ${
                isActive 
                  ? 'text-accent' 
                  : 'text-muted'
              }`}
